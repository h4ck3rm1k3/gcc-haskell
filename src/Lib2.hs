...
-- --  parseFileCallBack   (NTriplesParserCustom Attoparsec ProcessRdf) = parseFileAttoparsecCallBack 
...
--handleResult callback res :: CallBack
data Foo2 x = Foo3 (x)
--handleResult :: (t -> x) -> t -> Foo2 x
handleResult res = case res of
     Fail _ _ err -> error err
     Partial f -> handleResult (f (T.encodeUtf8 T.empty)) 
     Done _ ts -> do
       Right $ mkRdf (catMaybes ts) Nothing (PrefixMappings Map.empty) 
...
-- --parseStringAttoparsec :: (Rdf a) => T.Text -> CallBack-> Either ParseFailure (RDF a)
-- parseStringAttoparsec bs = do
--   let e = T.encodeUtf8 bs
--   let x = parse nt_ntripleDoc e
--   handleResult x
-- --parseStringAttoparsec callback bs = 1

data Foo x= Foo2(x)
--data CallBack =  ProcessRdf(Triple -> Foo Triple)

--parseFileAttoparsecCallback :: (Rdf a) => String -> CallBack -> IO (Either ParseFailure (RDF a))

--parseFileAttoparsecCallback2 callback x3  = do
--  parseStringAttoparsec callback x3
...
--parseFileAttoparsecCallback path  = parseFileAttoparsec path


  
  --x2
  --let x4 = x3 parseStringAttoparsec callback <$> TIO.readFile path

--parseFileAttoparsecCallback path callback= do
--  let f = <$> TIO.readFile path
--  let g = parseStringAttoparsec callback  f
--  g
...
--parseFileAttoparsecCallBack :: (Rdf a) => String -> ((Rdf a) -> (Rdf a))-> IO (Either ParseFailure (RDF a))
--parseFileAttoparsecCallBack path callback = parseStringAttoparsec <$> TIO.readFile path
